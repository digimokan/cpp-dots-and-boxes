...
TEST_CASE("calc_player_score(player, board)") {
  ...
  SUBCASE("1 x 1 board, empty") {
    ...
    RandomBoxScore scorer{ dimensions };
    Board board{ dimensions };
    const int64_t exp_player_one_score{ 0 };
    const int64_t exp_computer_score{ 0 };
    CHECK_EQ(scorer.calc_player_score(Player::ONE, board), exp_player_one_score);
    CHECK_EQ(scorer.calc_player_score(Player::COMPUTER, board), exp_computer_score);
  }
  ...
  SUBCASE("1 x 1 board, filled by one player") {
    ...
    for (std::size_t i{0}; i < board.get_max_lines(); ++i)
      board.mark_line(Player::ONE, i);
    const int64_t exp_player_one_score{ scorer.get_random_box_score(0) };
    const int64_t exp_computer_score{ 0 };
    CHECK_EQ(scorer.calc_player_score(Player::ONE, board), exp_player_one_score);
    CHECK_EQ(scorer.calc_player_score(Player::COMPUTER, board), exp_computer_score);
  }
  ...
  SUBCASE("2 x 2 board, three boxes filled by two players") {
    ...
    board.mark_line(Player::COMPUTER, 10);
    board.mark_line(Player::ONE, 3);
    const int64_t exp_player_one_score{
      scorer.get_random_box_score(0) + scorer.get_random_box_score(1)
    };
    const int64_t exp_computer_score{
      scorer.get_random_box_score(2)
    };
    CHECK_EQ(scorer.calc_player_score(Player::ONE, board), exp_player_one_score);
    CHECK_EQ(scorer.calc_player_score(Player::COMPUTER, board), exp_computer_score);
